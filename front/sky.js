...
(function () {
  ...
  async function loadAurora(quiet) {
    ...
    try {
      ...
      toast("Aurora nowcast unavailable (NOAA SWPC did not answer)", 4500, "error");
      state.aurora = false;
    ...
    if (!quiet) {
      ...
      toast(`Aurora: NOAA SWPC OVATION nowcast for ${hhmm(meta.forecast_time)}Z, peak ${meta.max_pct}% chance.${kp}`
