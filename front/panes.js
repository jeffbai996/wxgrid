...
(function () {
  ...
  function renderResort(pt, d, i) {
    ...
    const gradeBars = gradeMeta.map(([keys, label, color, shape]) => {
      const n = keys.reduce((sum, key) => sum + (grades[key] || 0), 0), pct = segments ? n / segments * 100 : 0;
      if (!n) return "";
      return `<div class="resort-grade"><span>${gradeMark(shape, color)}${label}</span><b>${n}</b><em><i style="width:${pct.toFixed(1)}%;background:${color}"></i></em></div>`;
    }).join("") || `<div class="note">No mapped difficulty tags.</div>`;
    const gradeExtras = northAmerican ? [[grades.freeride || 0, "off-piste"], [grades.unknown || 0, "unrated"]].filter(([n]) => n).map(([n, label]) => `${n} ${label}`).join(" · ") : "";
    const gradeFoot = gradeExtras ? `<p class="resort-grade-foot">${gradeExtras}</p>` : "";
